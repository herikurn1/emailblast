...
<body>
            ...
            <div class="headline-drafts h-100">
                ...
                <div class="contents2 h-auto" style="min-height: 67vh;">
                    <div class="login-form mt-0 p-5 h-100 bg-transparent">
                        <form id="form-create-email" action="/create/send" method="POST">
                            @csrf
                            <input type="number" id="id" name="id" value="{{ $mail->id ?? 0 }}" hidden required>
                            <div class="form-group">
                                <input type="title" name="title" class="form-control w-100" id="inputTitle"
                                    value="{{ $mail->title ?? '' }}" placeholder="Title" required>
                            </div>
                            <div class="form-group">
                                <input type="subject" name="subject" class="form-control w-100" id="inputSubject"
                                    value="{{ $mail->subject ?? '' }}" placeholder="Subject" required>
                            </div>
                            <div class="form-group">
                                <textarea name="Article_content" id="Article_editor" value="{{ $mail->body ?? '' }}" required></textarea>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div class="contents3">
                    <div class="row mt-4 plus float-right" style="margin-right: 1px;">
                        <div class="col">
                            <button class="col btn btn-green-moon btn-rounded" style="font-weight: bold">
                                Save as Draft
                            </button>
                        </div>
                        <div class="col">
                            <button id="btn-submit-form-create-email" type="submit" form="form-create-email"
                                class="col btn btn-orange-moon btn-rounded" style="font-weight: bold">
                                Next
                            </button>
                        </div>
                    </div>
    ...
    <script>
        ...
        CKEDITOR.instances['Article_editor'].setData('{!! addslashes($mail->body ?? "") !!}');
